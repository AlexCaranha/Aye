...
from libs.baseclass.dialog_change_theme import AyeDialogChangeTheme
from libs.baseclass.list_items import AyeOneLineLeftIconItem
from libs.baseclass.expansionpanel import AyeExpansionPanelContent

if getattr(sys, "frozen", False):  # bundle mode with PyInstaller
    os.environ["Aye_ROOT"] = sys._MEIPASS
else:
    sys.path.append(os.path.abspath(__file__).split("demos")[0])
    os.environ["Aye_ROOT"] = os.path.dirname(os.path.abspath(__file__))
os.environ["Aye_ASSETS"] = os.path.join(
    os.environ["Aye_ROOT"], f"assets{os.sep}"
)
...
class AyeApp(MDApp):
    ...
    def build(self):
        Builder.load_file(
            f"{os.environ['Aye_ROOT']}/libs/kv/list_items.kv"
        )
        Builder.load_file(
            f"{os.environ['Aye_ROOT']}/libs/kv/dialog_change_theme.kv"
        )
        return Builder.load_file(
            f"{os.environ['Aye_ROOT']}/libs/kv/start_screen.kv"
        )

    def show_dialog_change_theme(self):
        if not self.dialog_change_theme:
            self.dialog_change_theme = AyeDialogChangeTheme()
    ...
    def on_start(self):
        ...
        Builder.load_file(
            f"{os.environ['Aye_ROOT']}/libs/kv/base_content.kv"
        )
        Builder.load_file(
            f"{os.environ['Aye_ROOT']}/libs/kv/toolbar.kv"
        )
        ...
        for name_item_example in data_screens:
            self.root.ids.backdrop_front_layer.data.append(
                {
                    "viewclass": "AyeOneLineLeftIconItem",
    ...
    def set_example_screen(self, name_screen):
        ...
        if not manager.has_screen(
            ...
            Builder.load_file(
                f"{os.environ['Aye_ROOT']}/libs/kv/{name_kv_file}.kv"
            )
    ...
    def show_demo_shrine(self, instance):
        ...
        def show_demo_shrine(interval):
            from kivy.animation import Animation
            from demos.Aye.studies.shrine.shrine import MDShrine
        ...
        path_to_logo = (
            f"{os.environ['Aye_ROOT']}/studies/shrine/data/images/shrine-white.png"
            if self.theme_cls.theme_style == "Dark"
            else f"{os.environ['Aye_ROOT']}/studies/shrine/data/images/shrine-dark.png"
        )
        logo = Image(
    ...
    def add_expansion_panel(self, card):
        content = AyeExpansionPanelContent()
        card.add_widget(
            MDExpansionPanel(
                icon=f"{os.environ['Aye_ASSETS']}avatar.png",
                content=content,
